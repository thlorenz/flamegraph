...
var func = {
  ...
  , samples: '(12,381 samples, 55.49%)'
  , rect_x: 535.2
  , rect_y: 417
  , rect_w: 654.8
  , rect_h: 15.0
  , rect_fill: 'rgb(241,125,3)'
  , text_x: 538.184886378916
  , text_y:427.5
}
...
function colorMap(paletteMap, colorTheme, func) {
  if (paletteMap[func]) return paletteMap[func];
  paletteMap[func] = color(colorTheme);
...
function processNodes(parsed) {
  ...
  function processNode(node) {
    ...
    return {
      ...
      , rect_w    : x2 - x1
      , rect_h    : y2 - y1
      , rect_fill : color(paletteMap, opts.colors)
      , showText  : showText
      , text      : text
      , text_x    : x1 + 3
      , text_y    : 3 + (y1 + y2) / 2
