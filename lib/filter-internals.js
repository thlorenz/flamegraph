...
function filterLine(l) {
  return l
    ...
    .replace(v8MidRegex, '')
    // if it's the last function in the stack removal is a bit different since no ; delimits the end
...
function removeLine(l) {
  return !(/\(root\)( |;\(program\))/).test(l); // v8-profiler generated cpuprofile - don't need program at all
}
